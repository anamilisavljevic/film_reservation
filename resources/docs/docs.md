...
The `home-routes` handler in the `film-reservation.routes.home` namespace
...
The `home-page` function will in turn call the `film-reservation.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `film-reservation.handler` namespace:
...
Request middleware functions are located under the `film_reservation.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `film-reservation.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `film-reservation.db.core` in some other namespace.
